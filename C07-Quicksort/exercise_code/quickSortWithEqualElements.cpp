 ...
 */
pair<int, int> QuickSort::threeWayPartition(int start, int end,
                                            vector<int> &data) {
  int pivot = data[start];
  int low = start;
  int high = end;
...
/**
 * The three way partition above does extra swaps for elements not equal to
 * pivot. The fast three way partition below does extra swaps for elements equal
 * to pivot to i. This algorithm does fewer swaps when the number of elements
 * equal to pivot is smaller.
 *
 * Ref:Algorithms, 4th Edition by Robert S. and Kevin W.
 *
 */

pair<int, int> QuickSort::fastThreePartition(int start, int end,
                                             vector<int> &data) {
  int i = start, j = end + 1;
  int p = start, q = end + 1;
  int v = data[start];
...
bool QuickSort::isSorted(vector<int>::iterator begin, int size) {
