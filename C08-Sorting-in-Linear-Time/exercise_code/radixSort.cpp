...
public:
  ...
  RandInt(int low, int high)
      : ge(chrono::system_clock::now().time_since_epoch().count()),
        dist(low, high) {}
...
RadixSort::RadixSort(int n, int powValue)
    : size(n), digitNum(powValue), sortData(vector<int>(n)),
      dataCopy(vector<pair<int, int>>(n)) {
