...
BinaryStdOut::BinaryStdOut(string filename) { open(filename); }
...
void BinaryStdOut::write(bool x) { writeBit(x); }

void BinaryStdOut::write(char x) { writeChar(x); }
...
void BinaryStdOut::close() {
  flush();
  fout.close();
  cout << "-------------------- BinaryStdOut Writing Closing "
          "--------------------"
       << endl;
}
