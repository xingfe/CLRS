...
public:
  ...
  T get_minimum() const { return B_TREE_FIND_MIN(bRoot); }

  T get_maximum() const { return B_TREE_FIND_MAX(bRoot); }
...
private:
  ...
  // PRE: x is a node on the B-tree T. The top level call is
  // B-TREE-FIND-MIN(T.root).
  {
    if (x == nullptr) {
    ...
    if (i <= pNode->n && key == pNode->keyValue[i]) //关键字key在节点pNode中
    {
      ...
      } else // pNode是个内节点
      {
        ...
        if (pChildPrev->n >= t) //节点pChildPrev中至少包含CHILD_MIN个关键字
        {
          ...
          return;
        } else if (pChildNext->n >=
                   t) //节点pChildNext中至少包含CHILD_MIN个关键字
        {
    ...
    } else //关键字key不在节点pNode中
    {
      ...
      if (pChildNode->n == t - 1)          //只有t-1个关键字
      {
        ...
        if (pLeft && pLeft->n >= t) //左兄弟节点至少有CHILD_MIN个关键字
        {
          ...
          if (!pLeft->isLeaf) {
            for (j = pChildNode->n + 2; j > 1;
                 --j) // pLeft节点中合适的子女指针移植到pChildNode中
            {
          ...
          ++pChildNode->n;
          pNode->keyValue[i] =
              pLeft->keyValue[pLeft->n]; // pLeft节点中的最大关键字上升到pNode中
          --pLeft->n;
        } else if (pRight && pRight->n >= t) //右兄弟节点至少有CHILD_MIN个关键字
        {
          ...
          ++pChildNode->n;
          pNode->keyValue[i] =
              pRight->keyValue[1]; // pRight节点中的最小关键字上升到pNode中
          --pRight->n;
          for (j = 1; j <= pRight->n; ++j) {
            pRight->keyValue[j] = pRight->keyValue[j + 1];
          }
          if (!pRight->isLeaf) {
            pChildNode->pChild[pChildNode->n + 1] =
                pRight
                    ->pChild[1]; // pRight节点中合适的子女指针移植到pChildNode中
