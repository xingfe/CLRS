...
class FordFulkerson {
  ...
  bool check(FlowNetwork G, int s, int t) {
    ...
    if (abs(mincutValue - value) > FLOATING_POINT_EPSILON) {
      cerr << "Max flow value = " << value
           << ", min cut value = " << mincutValue << endl;
      return false;
    }
  ...
  // if so, upon termination edgeTo[] will contain a parent-link representation
  // of such a path this implementation finds a shortest augmenting path (fewest
  // number of edges), which performs well both in theory and in practice
