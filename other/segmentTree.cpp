...
class NumArray {
  ...
  SegmentTreeNode *build(int start, int end, const vector<int> &A) {
    if (start > end)
      ...
      SegmentTreeNode *tree =
          new SegmentTreeNode(start, end, left->sum + right->sum);
      tree->left = left;
      tree->right = right;
      return tree;
    }
  ...
  int query(SegmentTreeNode *root, int i, int j) {
    ...
    if (j <= root->left->end) {
      ...
      return query(root->left, i, root->left->end) +
             query(root->right, root->right->start, j);
    }
  }

public:
  NumArray(vector<int> &nums) { root = build(0, nums.size() - 1, nums); }

  void update(int i, int val) { modify(root, i, val); }
